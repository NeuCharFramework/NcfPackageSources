...
namespace Senparc.Ncf.Service
{
    ...
    public class SysMenuService : ClientServiceBase<SysMenu>
    {
        ...
        public virtual async Task<SysMenu> CreateOrUpdateAsync(SysMenuDto sysMenuDto)
        {
            ...
            {
                throw new NCFeExceptionBase($"ResourceCode：{sysMenuDto.ResourceCode}已重复");
            }
        ...
        public void Init()
        {
            IEnumerable<SysMenu> sysMenus = new List<SysMenu>()
            {
                ...
                new SysMenu(){ Id = "5", MenuName = "模块管理", Url = "/Admin/XncfModule/Index", Icon = "fa fa-user-secret", Visible = true, IsLocked = true, Sort = 175, ParentId = "4", MenuType = MenuType.菜单},
