...
{
    public class TerminalAppService : AppServiceBase
    {
        public TerminalAppService(IServiceProvider serviceProvider) : base(serviceProvider)
        {
        }
            ...
            return this.GetResponse<AppResponseBase<string>, string>((response, logger) =>
            {
                ...
                switch (upperCmd)
                {
                    ...
                    default:
                        ...
                        {
                            strExecRes = ExeCommand($"{request.CommandLine}");
                        }
                        else
                        {
                        ...
                        logger.Append(strExecRes);
                        response.Data = "操作成功！";

                        if (!string.IsNullOrEmpty(strExecRes))
                        {
                            response.Data += Environment.CommandLine + strExecRes;
                        }
