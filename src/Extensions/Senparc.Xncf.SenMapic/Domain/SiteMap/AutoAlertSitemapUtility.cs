    ...
    public class AutoAlertSitemapUtility
    {
        ...
        private void BuildOnlyOnceEventHandler(object state, bool timeout)
        {
            try
            {
                ...
                LogUtility.WebLogger.Debug("Sitemap预定总数:{0}，符合条件列队数:{1}，实际处理数：{2}".With(siteMapOrders.Count, listedAllOrdersCount, queueOrders.Count));
                ...
                while (remainQueueOrderCount > 0)
                {
                    if (dtStartWait.AddMinutes(5) <= DateTime.Now)
                    {
                        ...
                        LogUtility.SitemapLogger.Debug("等待线程结束已有5分钟。",
                            new Exception("总列队数:{0},使用中的线程数量:{1}".With(queueOrders.Count, autoAlertThreadInUsing)));
                    }
                ...
                //收录时间较长，则记录
                if (queueOrders.Count > 0 && (DateTime.Now - dtStartWait).TotalMinutes >= /*平均每个站点5分钟*/queueOrders.Count * 5)
                {
                    LogUtility.SitemapLogger.Debug("所有站点线程结束,耗时{0}分钟。".With((DateTime.Now - dtStartWait).TotalMinutes.ToString("#.#")),
                        new Exception("总列队数:{0},使用中的线程数量:{1}".With(queueOrders.Count, autoAlertThreadInUsing)));
                }

                if (autoAlertThreadInUsing > 0)
                {
                    LogUtility.SitemapLogger.Error("AutoAlert线程超出最大值，当前线程数：{0}。".With(autoAlertThreadInUsing));
                }
            }
            catch (Exception e)
            {
                LogUtility.SitemapLogger.Error("AutoAlertSitemap严重异常:" + e.Message + "。下一个AutoAlert循环将继续进行。", e);
                if (BuildOnlyOnce)
                {
                    throw;
                }
