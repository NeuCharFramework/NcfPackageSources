var app = new Vue({
  ...
  data() {
    return {
      ...
      graphDialogVisible: false, // 关系图谱弹窗
      graphCanvas: null,
      graphCtx: null,
      graphNodes: [],
      graphEdges: [],
  ...
  methods: {
    ...
    handleElVisibleOpenBtn(btnType, formData) {
      ...
      if (btnType === 'graph') {
        this.visible.graphDialogVisible = true;
        this.$nextTick(() => {
    ...
    initGraphCanvas() {
        ...
        // 添加鼠标事件监听
        this.graphCanvas.addEventListener('mousedown', this.handleCanvasMouseDown);
        this.graphCanvas.addEventListener('mousemove', this.handleCanvasMouseMove);
        this.graphCanvas.addEventListener('mouseup', this.handleCanvasMouseUp);
        this.graphCanvas.addEventListener('wheel', this.handleCanvasWheel);
        this.graphCanvas.addEventListener('contextmenu', this.handleCanvasContextMenu);
    ...
    loadGraphData() {
        // 从当前组加载智能体数据
        this.graphNodes = this.groupForm.members.map(member => ({
            id: member.id,
            name: member.name,
            x: Math.random() * (this.graphCanvas.width - 200) + 100,
            y: Math.random() * (this.graphCanvas.height - 200) + 100
        }));
        this.graphEdges = []; // 这里可以从后端加载已有的关系数据
        this.drawGraph();
    },
    ...
    drawGrid() {
        ...
        this.graphCtx.strokeStyle = '#2a2a2a';
    ...
    drawNode(node) {
        this.graphCtx.save();
        this.graphCtx.translate(node.x, node.y);
        
        // 绘制节点背景
        this.graphCtx.beginPath();
        this.graphCtx.arc(0, 0, 30, 0, Math.PI * 2);
        this.graphCtx.fillStyle = 'rgba(42, 130, 228, 0.8)';
        this.graphCtx.fill();
        this.graphCtx.strokeStyle = '#2a82e4';
        this.graphCtx.lineWidth = 2;
        this.graphCtx.stroke();
        
        // 绘制节点文字
        this.graphCtx.fillStyle = '#fff';
        this.graphCtx.font = '14px Arial';
        this.graphCtx.textAlign = 'center';
        this.graphCtx.textBaseline = 'middle';
        this.graphCtx.fillText(node.name, 0, 0);
    ...
    drawEdge(edge) {
        ...
        this.graphCtx.save();
        this.graphCtx.beginPath();
        this.graphCtx.moveTo(sourceNode.x, sourceNode.y);
        this.graphCtx.lineTo(targetNode.x, targetNode.y);
        ...
        if (!edge.bidirectional) {
            ...
            this.graphCtx.beginPath();
            this.graphCtx.moveTo(targetNode.x, targetNode.y);
            this.graphCtx.lineTo(
    ...
    handleCanvasMouseDown(e) {
        ...
        if (this.selectedNode) {
            ...
            this.tempEdge = { source: null, target: null, startX: x, startY: y };
        }
    },
    ...
    handleCanvasMouseMove(e) {
        if (this.isDragging && this.selectedNode) {
            ...
            this.drawGraph();
        } else if (this.isDrawingEdge && this.tempEdge) {
            const rect = this.graphCanvas.getBoundingClientRect();
            const x = e.clientX - rect.left;
            const y = e.clientY - rect.top;
            
            this.drawGraph();
            this.drawTempEdge(x, y);
        }
    },
    ...
    handleCanvasMouseUp(e) {
        if (this.isDragging) {
            ...
            // 检查是否释放到节点上
            const targetNode = this.graphNodes.find(node => {
                const dx = x - node.x;
                const dy = y - node.y;
                return Math.sqrt(dx * dx + dy * dy) <= 30;
            });
    ...
    handleCanvasContextMenu(e) {
        ...
        // 检查是否右键点击了边
        this.selectedEdge = this.graphEdges.find(edge => {
            const sourceNode = this.graphNodes.find(n => n.id === edge.source);
            const targetNode = this.graphNodes.find(n => n.id === edge.target);
            ...
            const t = ((x - sourceNode.x) * dx + (y - sourceNode.y) * dy) / (length * length);
    ...
    drawTempEdge(x, y) {
        this.graphCtx.beginPath();
        this.graphCtx.moveTo(this.tempEdge.startX, this.tempEdge.startY);
    ...
    addUnidirectionalEdge() {
        if (this.selectedEdge) {
            this.selectedEdge.bidirectional = false;
            this.drawGraph();
        }
    ...
    addBidirectionalEdge() {
        if (this.selectedEdge) {
            this.selectedEdge.bidirectional = true;
            this.drawGraph();
        }
        this.contextMenuVisible = false;
    },
    ...
    deleteEdge() {
        if (this.selectedEdge) {
            const index = this.graphEdges.indexOf(this.selectedEdge);
            if (index > -1) {
