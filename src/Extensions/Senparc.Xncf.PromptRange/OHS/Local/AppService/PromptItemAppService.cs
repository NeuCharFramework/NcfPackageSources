    ...
    public async Task<AppResponseBase<PromptResultDto>> Add(PromptItem_AddRequest request)
    {
      return await this.GetResponseAsync<AppResponseBase<PromptResultDto>, PromptResultDto>(
        async (response, logger) =>
        {
          ...
          var modelName = LlmModelHelper.GetRealModelName(llmModel.Name);
          ...
          {
            case Constants.ModelTypeEnum.AzureOpenAI:
              //todo: validate the params
              aiUrl = $"https://{llmModel.Endpoint}/{modelName}/{func}?api-version={llmModel.ApiVersion}";
              break;
            case Constants.ModelTypeEnum.HuggingFace:
              aiUrl = $"https://{llmModel.Endpoint}/{func}?api-version={llmModel.ApiVersion}";
              break;
            case Constants.ModelTypeEnum.OpenAI:
          ...
          client.Timeout = new TimeSpan(100 * 60 * 1000);

          // todo 改写为CO2NET的方法
          var dt1 = SystemTime.Now;
          var resp = await client.PostAsJsonAsync(aiUrl,
            new StringContent(promptItem.Content, encoding: Encoding.UTF8, "application/json"));
          logger.Append($"{resp}");
          
          //准备运行
          var userId = "Test"; //区分用户
          // handler.ChatConfig(promptParameter, userId,);
          
          // Senparc.CO2NET.HttpUtility.Post.PostGetJsonAsync(base.ServiceProvider, aiUrl, formData:,
          //   encoding: Encoding.UTF8, contentType: "application/json", timeOut: 100 * 60 * 1000);
          // var frameworkresult = await Senparc.CO2NET.HttpUtility.RequestUtility.HttpPostAsync(
          //     base.ServiceProvider,
          //     aiUrl, null, postStream: ms, contentType: "application/json", timeOut: 100 * 60 * 1000);
          // var frameworkerr = frameworkresult.GetObject<FastChatErrorResponse>();
          ...
          var promptResult = new PromptResult(
            request.ModelId, resp.Content.ToString(), SystemTime.DiffTotalMS(dt1), 0, 0,
            null, false, TestType.Text,
