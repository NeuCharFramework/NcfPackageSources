...
### Modify files
1. `tools/NcfSimulatedSite/Senparc.Areas.Admin/Areas/Admin/Pages/Index.cshtml` - Drag and drop area added in Step 03
...
**Modify location**: Enhance the existing `initializeModuleDrag()` method (Basic version has been added in Step 03)
...
- Use `dataTransfer` to transfer drag and drop data
...
**Modification**: Add module UID processing in the `OnGet` method
```csharp
...
**Add method**: Add module loading logic in `mounted()`
```javascript
mounted() {
  // 从页面获取初始数据
...
- The `dragover` event of the drag and drop area must `preventDefault()`, otherwise `drop` will not be triggered
