...
- Inherited from `EntityBase<int>`, automatically obtains Id, TenantId, AddTime, LastUpdateTime, Flag and other fields
- Use the `[Table]` attribute to specify the table name, which must be prefixed with `Register.DATABASE_PREFIX`
- All setters use `private set` and are modified through public methods, in line with the principles of domain-driven design
- Use `private` constructor for EF Core use and public constructor for business creation
...
```**Key technical points**:
- Inherited from `DtoBase<int>`, automatically obtains the Id attribute of type int
- At the same time inherit basic fields such as Flag, AddTime, LastUpdateTime, TenantId, etc.
- Use the nullable type `bool?` to represent three states (like/dislike/no feedback)
- `CreatedTime` independent field facilitates query and sorting
- The `Sequence` field ensures the order of messages
- Provide simplified `ChatMessageInputDto` for front-end submission (no need to inherit DtoBase)
...
**Modify location**: Add before the line `//DOT REMOVE OR MODIFY THIS LINE`
...
- [ ] All DTOs inherit from `DtoBase<int>` (Id attribute automatically provided)
- [ ] Table names use the `[Table]` attribute and add a prefix
- [ ] All required fields marked `[Required]`
- [ ] Set a reasonable `MaxLength` for string fields
- [ ] Foreign key relationships use the `[ForeignKey]` attribute
...
⚠️ **Important**:
- Must use `Register.DATABASE_PREFIX` as table name prefix to avoid conflicts with other modules
- All attributes of the entity must use `private set` and be modified through public methods
- **DTO must inherit `DtoBase<int>`**, which will automatically provide the Id attribute of int type without manual definition.
...
- The DbSet in AdminSenparcEntities must be added before the `//DOT REMOVE` comment
