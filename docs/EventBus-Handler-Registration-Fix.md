...
- **root cause**:`PromptInitResponseHandler`It is not registered to the DI container, causing EventBus to be unable to call it to complete the initialization request.
...
- **Root Cause**: Direct access from front-end code`response.data.models`, but NCF’s`AppResponseBase`The return format is nested`{ success: true, data: { models: [...] } }`
...
**Problem Analysis**:
The previous code only registered EventBus itself and HostedService, but did not call`AddSenparcEventBus()`Method to scan and register EventHandlers.
...
**Key Improvements**:
1. Use`AddSenparcEventBus()`Method to replace manual registration
2. Automatically scan all`Senparc.Xncf.*`and`Senparc.Areas.*`assembly
3. Configure EventBus options (concurrency, loop detection, retry, etc.)
4. in`StartWebEngine`**after** call to ensure all module assemblies are loaded
...
**Improvement points**:
- added`console.log`Output the complete response for easy debugging
- Correct access to nested`data.data`structure
...
### Why Pending?
1. Controller is used in lines 86-87`Task.WhenAny(tcs.Task, timeoutTask)`Waiting for response
2. Respond to needs`PromptInitResponseHandler`call`CompleteInitRequest()`to complete
3. if`PromptInitResponseHandler`has not been registered,`PromptInitResponseEvent`will not be processed
4. `tcs.Task`Never completes, the request remains pending until it times out in 2 minutes
...
1. **Auto Scan**:`AddSenparcEventBus()`Use reflection scanning to specify an assembly
2. **Interface matching**: Search is implemented`IIntegrationEventHandler<T>`class
3. **DI Registration**: Register Handler as Scoped service
4. **Dynamic call**: HostedService passes`GetServices(handlerType)`Get Handler instance
...
All NCF API return formats follow`AppResponseBase<T>`structure:
```typescript
{
  success: boolean,
  data: T,                  // 实际数据在这里
...
Must be used when accessing data on the front end`response.data.data.xxx`(Two layers of data).
...
4. **Restore permission verification**: After passing the functional test, uncomment all`[ApiAuthorize("AdminOnly")]`property
...
- `src/Extensions/Senparc.Xncf.PromptRange/wwwroot/js/PromptRange/prompt.js`- Front-end API call logic
