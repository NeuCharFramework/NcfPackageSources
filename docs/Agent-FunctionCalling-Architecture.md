...
- **Before**: The AI ​​optimization process was "solidified" in`PromptOptimizationRequestHandler`, call AI Kernel directly
...
- Immediately after **creating a ChatTask** **start** it (calling`ChatGroupService.RunChatGroupInThread`）
- **Build Task Instructions**(`BuildAgentCommand`), tell the Agent exactly what to do, including:
- Mission objectives
- User needs
- Current information
- Perform steps (1-7)
...
- Add to`BuildAgentCommand`method
...
### Preconditions:
1. **Restart the application** (required, make sure the Plugin is correctly registered)
   ```bash
# Press Ctrl+C in terminal 5 to stop (if running)
...
- Or update the Agent`FunctionCallNames`Field
...
- For example:
     ```
Agent: I need to get the current Prompt information first
...
Agent: Next analyze the historical ratings
     [Function Call: AnalyzeModelScores(rangeName="2025.12.28.3")]
     [Function Result: Model Performance Analysis: ...]
     
Agent: Based on the analysis, I decided to use ModelId=5 and optimize the Prompt content
...
- **solve**:
1. Check the Agent in the database`FunctionCallNames`Field
2. Make sure you restart the app
3. Check the console log for`FunctionCallNames`output
...
- **solve**:
1. Check`BuildAgentCommand`The output of
...
- **solve**:
1. Check`Register.cs`Two registrations in
2. Restart the application

### 4. ChatTask is always in Waiting state
- **reason**:`RunChatGroupInThread`not called
- **solve**:
1. Check`PromptOptimizationChatTaskHandler`code
...
### 3. **Extensibility**
- Easily add new functions (eg: AnalyzePromptQuality, SuggestImprovements)
...
| Task instruction construction | `PromptOptimizationChatTaskHandler.BuildAgentCommand()` |
