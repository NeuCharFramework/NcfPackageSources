...
- **document**:`PromptOptimizationRequestHandler.cs` - `SelectBestModelIdAsync`method

#### 🔧 Function 2: JSON escape character processing
- **Problem**: The content returned by AI contains`\n`string instead of actual line breaks
- **Fix**: Added`UnescapeJsonString`Methods handling JSON escaping
- **support**:`\n`, `\r`, `\t`, `\"`, `\\`
- **document**:`PromptOptimizationRequestHandler.cs` - `UnescapeJsonString`method
...
- rear end:`PromptOptimizationEvents.cs`, `PromptOptimizationRequestHandler.cs`
...
- Add to`autoShootAfterOptimize`and`autoAIGradeAfterShoot`data fields
- Revise`executeOptimize`Method to pass options to the backend
...
- The second checkbox depends on the first (`:disabled="!autoShootAfterOptimize"`）
- Add Tooltip description

### Backend files

3. **`src/Extensions/Senparc.Xncf.PromptRange.Abstractions/Events/PromptOptimizationEvents.cs`**
- exist`OptimizationContext`Add two optional parameters to record:
...
- New`SelectBestModelIdAsync`Method (intelligent ModelId selection)
- New`UnescapeJsonString`Method (JSON escape character processing)
...
- exist`OptimizeAsync`Add in method`EnsureInitializedAsync()`call
...
7. **`docs/PromptCatalyzer-Smart-Optimization.md`** - Intelligent optimization enhances documentation
8. **`docs/PromptCatalyzer-Auto-Shoot-And-Grade.md`** - Automatic target shooting and scoring documentation
...
**verify**:
- The console log shows:`Smart selection ModelId: original=1012, select=1011`
- Show rating statistics:`Range model rating statistics: Model1011=0.89 (5 times), Model1012=0.82 (3 times)`
...
**verify**:
- The console displays:`[Step 3/3] Check whether ChatGroup already exists...`
- The console displays:`✅ ChatGroup created successfully! `
- can be found in the database`PromptCatalyzer-OptimizationGroup`
...
**Reason**: PromptItem`ExpectedResultsJson`Field is empty
...
3. View the actual contents of the Content field in the database (use`SELECT CONVERT(varbinary(max), Content) ...`）
...
- current`[ApiAuthorize]`Commented out
...
| stage | Function | degree of automation |
|------|------|-----------|
...
| **Model Selection** | Choose the best model based on historical ratings | 🟢 Fully automatic |
| **Version Management** | Automatically increment version number (Aiming) | 🟢 Fully automatic |
| **Target Test** | Automate test execution | 🟡 Optional (enabled by default) |
