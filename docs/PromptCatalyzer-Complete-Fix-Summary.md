...
- use`AddSenparcEventBus()`Automatically scan and register EventHandler
...
- initialized in constructor`NickName = string.Empty;`

3. **`src/Extensions/Senparc.Xncf.PromptRange/Application/EventHandlers/PromptInitRequestHandler.cs`**
- Will`Note`fields from`"Auto-created for PromptCatalyzer initialization with Model ID: {modelId}"`Change to`"AI-Catalyzer"`(Meets 20 character limit)
...
6. **`src/Extensions/Senparc.Xncf.AgentsManager/OHS/Remote/Controllers/PromptOptimizationController.cs`** ⭐ New document
- Create a replacement for traditional MVC Controller`PromptOptimizationAppService`
- accomplish`OptimizeAsync`method
...
- Fixed`executeInitialization`Data access path
...
If you need to use the "PromptCatalyzer-T1-A1" format, you need to modify it`PromptRangeService.AddAsync()`Or specify RangeName when creating the Range.
...
After successful initialization, the database should have:
...
2. **Server log** (including the part of optimization request)
   ```bash
grep -A 20 "Prompt optimization request received" tools/NcfSimulatedSite/Senparc.Web/App_Data/SenparcTraceLog/SenparcTrace-*.log
   ```
...
1. **Permission Verification**: All AppService`[ApiAuthorize("AdminOnly")]`Annotated and needs to be re-enabled
2. **ApiBind Mechanism Survey**: Why`[ApiBind]`Not working in AgentsManager project?
