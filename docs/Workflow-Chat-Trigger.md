...
- 工作流仍完全由服务端运行协调器执行，浏览器端不参与运行，会话历史保存在进程内存中。
...
| POST | `{workflowId}/reset` | 清空当前会话（不中止进行中的运行） |
...
| `Domain/Services/NeuCharWorkflowChatSessionService.cs` | 进程内会话存储：消息历史（≤200 条/会话）、运行占位（一会话一运行）、24 小时 TTL、500 会话上限 |
...
| 文件 | 变更 |
| --- | --- |
| `Domain/Services/NeuCharWorkflowEngine.cs` | 允许 `chat-trigger` 节点；执行时透传输入；输出描述符按 `string` 处理 |
| `Domain/Services/NeuCharWorkflowRunCoordinator.cs` | 运行来源新增 `chat` |
| `Application/AppServices/NeuCharWorkflowAppService.cs` | 保存时规范化 Chat 配置；新增 `GetChatBootstrapAsync` / `SendChatMessageAsync` / `GetChatRunStatusAsync` / `ResetChatSessionAsync` 与对应结果记录 |
| `Register.cs` | 注册 `NeuCharWorkflowChatSessionService`（Singleton） |
...
1. 用户发送消息 → `SendChatMessageAsync` 校验（模块/工作流/触发方式/访客）→ 追加用户消息 → `RunCoordinator.TryStart(..., source: "chat")` → 会话记录 `PendingRunId`。
2. 页面每 1.2 秒轮询 `GetChatRunStatusAsync` → 运行中显示最新节点事件；结束时把最终输出（或错误）写入会话历史并释放占位。
3. 页面刷新后通过 bootstrap 的 `pendingRunId` 恢复轮询。
4. 聊天输入按纯文本字符串传给触发器（与手动触发一致，`parseInputAsJson=false`）。

## 🔒 安全说明

- 入口控制器 `[AllowAnonymous]`，但每次调用都校验工作流启用状态、触发方式与访客开关；工作流未启用时不可聊天。
- 访客令牌仅作为会话标识，不携带任何凭据；令牌哈希后作为内存字典键，服务端日志与 API 不返回原始令牌。
- 消息长度上限 8000 字符；会话历史与运行快照均为进程内存，重启后自动清空（与运行协调器、Human Input 服务一致）。
