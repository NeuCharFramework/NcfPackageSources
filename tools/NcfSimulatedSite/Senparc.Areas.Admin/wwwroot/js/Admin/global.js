...
function formaTableTime(value) {
    return value ? value.replace('T', '  ').substr(0, 17) : ncfT('Common.NoTime');
}
...
function copyToClipboard(value, toastText) {
    ...
    toastText = toastText || ncfT('Common.CopySuccess');
    try {
...
(function () {
    ...
    function getI18n() {
        return window.NCF_SESSION_I18N || {
            title: ncfT('Admin.Session.ExpiringTitle'),
            message: ncfT('Admin.Session.ExpiringMessage'),
    ...
    function showReminderDialog() {
        ...
        app.$confirm(
            i18n.message || ncfT('Admin.Session.ExpiringMessage'),
            i18n.title || ncfT('Admin.Session.ExpiringTitle'),
            {
