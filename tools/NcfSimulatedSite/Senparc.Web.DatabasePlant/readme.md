...
1. This project is for use`dotnet ef migrations add`When using the command, specify a startup project with a specific target framework (such as netcoreapp3.1 or net6.0), so that projects with any target framework (such as netstandard2.1) can be operated without any modification;
