...
**Solution**:
- exist`MainWindow.axaml.cs`added in`AdjustWindowSizeToScreen()`method
...
**Solution**:
- Using Avalonia`ThemeDictionaries`System-defined light and dark theme resources
- Replaced all hardcoded colors with dynamic resource references`{DynamicResource ...}`
...
### exist`App.axaml`Global theme resources defined in:

| Resource name | light mode | dark mode | use |
|---------|---------|---------|------|
| `CardBackgroundBrush` | #FFFFFF (white) | #2B2B2B (Dark Grey) | card background |
...
| `ToolbarBorderBrush` | #DEE2E6 (grey) | #3C3C3C (medium gray) | toolbar border |
| `InputBackgroundBrush` | #FFFFFF (white) | #3C3C3C (medium gray) | Input box background |
| `InputBorderBrush` | #CED4DA (medium gray) | #555555 (grey) | Input box border |
...
- ✅ Update`Border.card`Styles, using dynamic resources
...
1. **Theme switching**: application usage`RequestedThemeVariant="Default"`, will automatically follow the system theme. If you need a fixed theme, you can change it to`"Light"`or`"Dark"`。

2. **Custom Color**: If you need to add a new theme color, please`App.axaml`of two`ResourceDictionary`added at the same time.
...
5. **Performance Impact**: Use`DynamicResource`There is a slight performance overhead, but it is negligible for desktop applications.
...
- confirm`MainWindow.axaml.cs`in`AdjustWindowSizeToScreen()`called
- Try manual adjustment`safetyMarginHeight`Constant (increase to 150 or 200)

### Problem: Colors are still incorrect in dark mode
**Solution**:
- Check if all colors are used`{DynamicResource ...}`
- confirm`App.axaml`Dark theme resources are defined in
- try at`App.axaml`Explicit settings in`RequestedThemeVariant="Dark"`test

### Problem: The application does not follow the system theme switching
**Solution**:
- confirm`RequestedThemeVariant="Default"`(instead of Light or Dark)
