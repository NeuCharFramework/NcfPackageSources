...
### Root cause:
1. **Timing issue**: `ScrollToBottomIfNeeded()` is called immediately after `LogText` is updated, but the UI content may not be fully rendered at this time
2. **Control search failed**: When the application starts, `_cachedScrollViewer` may be null, and the UI may not be fully loaded, resulting in ScrollViewer not being found.
...
Create the `ScrollToBottomIfNeededDelayed()` method to ensure that the UI content is rendered before scrolling:```csharp
...
Change all `ScrollToBottomIfNeeded()` calls to `ScrollToBottomIfNeededDelayed()`:

- ✅ `OnLogUpdateTimerElapsed()` - when the timer is updated in batches
- ✅ `FlushPendingLogs()` - when app is ready or stopped
...
###Main changes:
1. ✅ Added `ScrollToBottomIfNeededDelayed()` method
2. ✅ Update `OnLogUpdateTimerElapsed()` to call delayed scrolling
3. ✅ Updated `FlushPendingLogs()` to call delayed scrolling
4. ✅ Keep `ScrollToBottomIfNeeded()` method (backup)
