...
**Success Flag**:
- `codesign`Output shows signature information
- `spctl`return`accepted`or`source=Developer ID`
...
create`~/.appstoreconnect/private_keys`Directory and configuration:
...
**Possible reasons**:
- used ad-hoc signatures (`-`) instead of a developer certificate
...
- Verify that the application package structure is complete
