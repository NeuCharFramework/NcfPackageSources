...
### Build a single file self-contained version for all platforms```bash
./build-tool/build-all-platforms-self-contained.sh --clean --single-file --ready-to-run
```### MacOS single file self-contained version```bash
...
- ✅ **Double-click to run**: Create standard `.app` application package
...
| Documentation | Platform | Description |
|------|------|------|
...
**If you need to double-click to run an application on macOS**, follow these steps:```bash
# 步骤 1：构建 macOS 可执行文件
...
```**One line of command to complete all operations**:```bash
...
```Note: The above self-contained scripts are always published using `--self-contained true` to facilitate running on devices that do not have dotnet-runtime installed.

### Ordinary release script (framework dependency)```bash
# 发布所有平台
...
| Parameters | Bash | Batch | PowerShell | Description |
|------|------|-------|------------|------|
| Help | `--help` | `/h` | `-Help` | Display help message |
| Cleanup | `--clean` | `/c` | `-Clean` | Clean the output directory before publishing |
| Specific platforms | `--platform <name>` | `/p <name>` | `-Platform <name>` | Publish only for specified platforms |
| Self-contained | `--self-contained` | `--self-contained` | `-SelfContained` | Contains the .NET runtime |
...
| Verbose output | - | - | `-Verbose` | Display verbose build information |

## 📦 Output structure

After publishing is complete, the self-contained version files will be saved in the `publish-self-contained` folder:```
...
├── linux-x64/
└── linux-arm64/
```The normal (framework dependent) version is still saved in the `publish` folder.

### 🍎 macOS application package output

The macOS application package generated using `create-macos-app.sh` is saved in the `macos-app/` folder:```
macos-app/
...
```**Usage Suggestions**:
- **Personal use**: Double-click `NCF Desktop-Universal.app` directly
...
3. **Permission Error (Linux/macOS)**```bash
   chmod +x build-tool/build-all-platforms.sh
   chmod +x build-tool/create-macos-app.sh
   ```4. **PowerShell execution policy error**```powershell
   Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser
   ```### 🍎 macOS special problem solving

5. **macOS cannot run when double-clicked**```bash
   # 使用应用程序包生成工具
...
9. **Application package signing issue**```bash
   # 手动重新签名（ad-hoc 签名）
...
A security warning about the `System.Text.Json` package may appear during the build process. This is a known issue and does not affect the normal operation of the application.
...
The application can be packaged as a single executable file using the `--single-file` parameter, but:
...
**Prerequisite**: Please run the self-contained release script first```bash
./build-tool/build-all-platforms-self-contained.sh -p osx-arm64    # Apple Silicon
./build-tool/build-all-platforms-self-contained.sh -p osx-x64     # Intel Mac
```**Application package generation**:```bash
# 基本使用：创建 .app 包
...
- ✅ **Double-click to run**: Generate a standard `.app` package and support double-click to start
...
The generated files are saved in the `macos-app/` directory:```
macos-app/
...
The following is the complete process from source code to double-click running application on macOS:```bash
# 1. 克隆或下载项目（如果还没有）
...
- **Personal use**: Use `NCF Desktop-Universal.app` directly
- **Distribute to Others**: Using `NCF Desktop-1.0.0.dmg`, recipients can simply double-click to install it
...
> To execute the Bash script, please grant executable permissions first: `chmod +x build-tool/build-all-platforms-self-contained.sh`
