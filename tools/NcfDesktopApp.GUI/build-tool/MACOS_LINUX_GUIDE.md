[中文版](MACOS_LINUX_GUIDE.cn.md)

# Guide to using macOS and Linux executables

## 📖 Overview

On Unix systems (macOS and Linux), executable files typically do not have an extension, which is completely normal behavior. Unlike Windows' `.exe`, Unix executable files are identified by file permissions rather than by extension.
...
```### Method 1: Run the executable file directly (simple test)```bash
# 1. 确保文件有可执行权限
...
#### Option A: Create automatically when publishing```bash
...
./build-tool/build-all-platforms-self-contained.sh \
    ...
    -p osx-arm64 \
    --create-app
```#### Option B: Create separately```bash
# 1. 先发布
...
| Features | Direct executables | .app packages |
|------|--------------|---------|
| Run mode | Command line | Double-click to open |
| Finder Integration | ❌ | ✅ |
| Dock Icon | ❌ | ✅ |
| App Icon | ❌ | ✅ |
...
### Advanced: Code Signing and Notarization (Release to Production)```bash
# 1. 创建签名的 .app 包
...
**A:** Install system dependencies:```bash
# Ubuntu/Debian
sudo apt-get install libicu-dev libssl-dev
...
### Release command quick check```bash
# Windows（生成 .exe 文件）
...
- **macOS application packaging**: `build-tool/create-macos-app.sh --help`
- **Single file release fix**: `build-tool/SINGLE_FILE_FIX.md`
