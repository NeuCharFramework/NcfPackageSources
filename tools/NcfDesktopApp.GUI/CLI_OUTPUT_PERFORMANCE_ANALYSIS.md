[中文版](CLI_OUTPUT_PERFORMANCE_ANALYSIS.cn.md)
...
| Operation | Overhead of each log | Total cost of 200 logs | Impact |
|------|--------------|----------------|------|
| Thread switching | 3 times | **600 times** | 🔴 Serious |
| String splitting | 1 time (O(n)) | **200 times** | 🔴 Serious |
| Control search | 1 time (visual tree traversal) | **200 times** | 🔴 Serious |
| UI redraw | 1 time | **200 times** | 🟡 Medium |
| Delay task | 1 time (10ms) | **200 times** (2 seconds total delay) | 🟡 Medium |
...
**Current implementation** (`ViewModels/MainWindowViewModel.cs:1099-1126`):```csharp
...
### Bottleneck 2: Frequent string operations 🔴 Serious```csharp
// 每次都要遍历整个字符串
var lines = _logBuffer.ToString().Split('\n');  // O(n)
...
**Current implementation** (`ViewModels/MainWindowViewModel.cs:1131-1167`):```csharp
...
```**Question**:
- ❌ Each log `FindControl<ScrollViewer>` → **Traverse the visual tree**
- ❌ 200 logs = **200 control searches**
- ❌ Control search overhead: ~1-5ms/time → **200-1000ms total delay**
- ❌ additional `Task.Delay(10)` → **2 seconds total delay**

---

### Bottleneck 4: Frequent UI redraw 🟡 Moderate```csharp
...
```**Optimization effect**:
| Metrics | Before | After Optimization | Improvement |
|------|------|--------|------|
| Thread switching | 600 times | ~10 times | **98% ↓** |
| String splitting | 200 times | ~2 times | **99% ↓** |
| Control search | 200 times | ~10 times | **95% ↓** |
...
**Problem**: `FindControl<ScrollViewer>` traverses the visual tree every time
...
**Idea**: Allow users to filter unimportant CLI output```csharp
...
| Solution | Thread switching | String splitting | Control search | UI redrawing | Estimated time consumption | Recommendation |
|------|----------|-----------|----------|--------|----------|--------|
...
### Complete optimization implementation```csharp
