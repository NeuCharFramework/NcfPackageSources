...
def add_rounded_corners_and_white_padding(image_path, output_path, corner_radius_ratio=0.18, padding_ratio=0.1):
    """
    为图片添加圆角和白色背景边距
    ...
    try:
        ...
        with Image.open(image_path) as img:
            ...
            # 创建白色背景图片
            new_img = Image.new('RGBA', (original_size, original_size), (255, 255, 255, 255))
            ...
            # 创建临时图片用于应用蒙版
            temp_img = Image.new('RGBA', (original_size, original_size), (0, 0, 0, 0))
            temp_img.paste(new_img, (0, 0))
...
def process_all_icons():
    ...
    if os.path.exists(backup_dir):
        ...
        for backup_file in os.listdir(backup_dir):
            ...
            if backup_file == "NCF-logo.png":
                target_path = os.path.join(base_dir, backup_file)
            else:
                target_path = os.path.join(base_dir, "AppIcon.iconset", backup_file)
            
            if os.path.isfile(backup_path):
                shutil.copy2(backup_path, target_path)
                print(f"📋 已恢复: {backup_file}")
        print()
    ...
    for input_file, output_file in files_to_process:
        ...
        # 处理图片
        add_rounded_corners_and_white_padding(input_path, output_path)
    
    print()
    print("🎉 所有图标处理完成!")
    print("📝 说明:")
    print("   - 使用白色背景匹配Logo背景色")
    print("   - 已为大于128px的图标添加了18%圆角和10%白色边距")
    print("   - 图标符合MacOS设计规范")
    print()
    print("🔄 重新生成.icns文件:")
