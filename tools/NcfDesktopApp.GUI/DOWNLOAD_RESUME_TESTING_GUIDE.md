[中文版](DOWNLOAD_RESUME_TESTING_GUIDE.cn.md)
...
- [ ] The `.download` file is automatically deleted after the download is completed.
...
3. Manually edit the `.download` file and modify the version number in the URL (simulating a new version)```
...
- [ ] Old files and `.download` files are deleted
...
2. Close the application
3. Manually delete the `.download` file (retaining the partially downloaded main file)
4. Restart the application
5. Click to download the NCF package
...
- [ ] Download restarts from 0%
- [ ] A new `.download` file is created
...
- [ ] The `.download` file is deleted after the download is completed
...
3. **Meta Information File**
   - User manual deletion of `.download` files will cause re-downloading
