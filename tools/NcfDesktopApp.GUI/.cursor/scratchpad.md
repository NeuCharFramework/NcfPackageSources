...
- Use different prefix/color identifiers (e.g. `[CLI]`)
...
1. **Process Output Capture**
   - `RedirectStandardOutput = true` and `RedirectStandardError = true` are set
...
2. **Thread Safety**
   ...
   - Use `Dispatcher.UIThread.Post` to update UI
   - Avoid using `Invoke` (synchronous call) to avoid blocking
...
| Abnormal process exit leads to event processing exception | 🟡 Medium | Application crashes | Add try-catch to handle process exit gracefully |
| Multi-thread race condition | 🟢 Low | Logs out of order | Use Dispatcher to ensure serial updates |
| Log encoding problem (Chinese garbled characters) | 🟢 Low | The log is unreadable | Set the correct console encoding (UTF-8) |
...
- [x] **[TASK-03]** Test and optimize performance (actual time taken: 0.3h)
  ...
  - File: `CLI_OUTPUT_TESTING_GUIDE.md`
...
3. **Thread Safety**: Use `Dispatcher.UIThread.Post` to ensure safe UI updates
...
- **Cause**: `ReadTimeout` is set in `AttachProcessOutputHandlers`, which conflicts with asynchronous event handling
- **Solution**: Remove the `ReadTimeout` setting and completely use event-driven asynchronous method
- **Fix Time**: 2025-11-16
...
- **Solution**:
  - Create a `.download` meta-information file to record the original download URL
...
2. **Breakpoint resume version verification**
   ...
   - Solution: Use `.download` file to store the original URL, and verify the version through URL comparison
   - Advantages:
...
```
   ...
   - Problem: The original code using `ReadToEndAsync()` will block until the process ends
   - Solution: Use event-driven `OutputDataReceived` + `BeginOutputReadLine()` instead
...
4. **Multi-threaded UI update**
   ...
   - Solution: Use `Dispatcher.UIThread.Post()` instead of `Invoke()`
...
5. **HTTP Range request processing**
   ...
   - Note: Use `FileMode.Append` to append and `FileMode.Create` to overwrite.
...
6. **Unified processing of three startup points**
   ...
   - Solution: Extract `AttachProcessOutputHandlers()` helper method
...
7. **Remember to clean up callbacks and temporary files**
   ...
   - Delete `.download` meta information file after download is complete
...
10. **⚠️ Avoid updating UI for every log**
   - ❌ Don’t `Dispatcher.Post` and redraw the UI for every log
...
11. **⚠️ Cache control references to avoid repeated searches**
   - ❌ Don’t traverse the visual tree with `FindControl<T>` every time
...
**Technical Highlights**:
- Real-time capture using `OutputDataReceived` event
- `Dispatcher.UIThread.Post` ensures thread safety
...
1. **ViewModels/MainWindowViewModel.cs**
   ...
   - Override `AddLog()` method (batch update)
   - Override `AddCliLog()` method (batch update)
   - Added `OnLogUpdateTimerElapsed()` method (timer callback)
   - Override `ScrollToBottomIfNeeded()` method (cache optimization)
   - Added `FlushPendingLogs()` method (refresh when stopped)
   - Modify `StopNcfAsync()` method (refresh before stopping)

**Breakpoint resume function (v1.1.0)**
2. **Services/NcfService.cs**
   - Implement `DownloadFileAsync` breakpoint resume function
   - Add version verification mechanism (`.download` meta information file)
   - Implement `DownloadToFileAsync` helper method
