...
- ✅ **Clear Marking**: Use`[CLI]`and`[CLI:ERROR]`Prefixes distinguish different types of output
- ✅ **THREAD SAFE**: Use`Dispatcher.UIThread.Post`Securing UI updates
...
- `OnProcessOutput`Callback properties (line 41)
- `AttachProcessOutputHandlers()`Helper methods (lines 953-1014)
...
**Modified content**:
- exist`StartNcfProcessAsync()`Register callbacks in (lines 717-721)
- exist`StopNcfAsync()`Cleanup callback (lines 749-753)
...
use`OutputDataReceived`Event rather than blocking reading:
...
Use asynchronous Post instead of synchronous Invoke:
...
All launch points have encoding set:
...
A: The current implementation is suitable for most scenarios. If you encounter lag, you can refer to`step-02-viewmodel-integration.md`Optimization scheme for batch updates in .
