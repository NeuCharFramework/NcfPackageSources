[中文版](VERSION_UPDATE_FEATURE.cn.md)
...
- 🔄 **AUTO-RECOVER**: Restore all data immediately after update
...
2. **Completely copy the App_Data folder**```
   ...
   ├── error.log         ← 错误日志
   └── ...               ← 其他日志文件
   ```Note: The `log/` folder is also supported (backwards compatible)
...
3. **🆕Smart recovery profile (with conflict detection)**
   ...
   - If the contents are the same, use the new configuration directly
   - If the content is different, a confirmation dialog box pops up to let the user choose:
     - **Use old configuration**: Keep custom settings and backup the new configuration as `appsettings.backup-yyyyMMdd-HHmmss.json`
...
#### Reserved file (ShouldPreserveFile)```csharp
// 1. App_Data 中的所有文件
...
if (fileName.StartsWith("appsettings") && fileName.EndsWith(".json"))
    return true;
```#### Reserved directory (ShouldPreserveDirectory)```csharp
// 1. App_Data 及其所有子目录
if (relativePath.StartsWith("App_Data"))
    return true;
...
- `SafeCleanRuntimeDirectoryAsync()` - Safely clean the directory
- `RestoreImportantFilesAsync()` - Restore important files
- `RestoreAppSettingsFilesAsync()` - 🆕 Intelligent recovery of configuration files (with conflict detection)
- `HandleAppSettingsConflictAsync()` - 🆕 Handle configuration file conflicts
- `ShouldPreserveFile()` - Determine whether to preserve files (including logs folder)
- `ShouldPreserveDirectory()` - Determine whether to preserve the directory (including the logs folder)
...
- `HandleAppSettingsConflictAsync()` - 🆕 Handle configuration file conflict dialog box
...
2. Manually copy `backup/App_Data` to `Runtime/App_Data`
3. Restart the application

#### Q: How to force a re-download?
**A**:
1. Stop NCF
2. Delete the `Runtime` folder
3. Delete the ZIP file in the `Downloads` folder
...
1. **🆕 logs folder protection**
   - Automatic backup and restore of `logs/` folder
   - Also supports `log/` folder (backwards compatible)
