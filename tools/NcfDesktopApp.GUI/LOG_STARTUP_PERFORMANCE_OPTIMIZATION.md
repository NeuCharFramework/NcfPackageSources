[中文版](LOG_STARTUP_PERFORMANCE_OPTIMIZATION.cn.md)
...
3. **A large amount of logs blocks the UI**: When there are thousands of lines of logs, `_logBuffer.ToString()` and UI rendering will block the main thread.
...
- **Prompt message**: Display `[X lines of startup log have been skipped, only the last 200 lines are displayed]`
...
#### 1. Application startup phase (not ready)```csharp
...
```#### 2. After the application is ready```csharp
// 标记应用已就绪
_isApplicationReady = true;
// 立即刷新一次日志显示（只显示最后200行）
FlushPendingLogs();
```#### 3. Dynamically adjust update frequency```csharp
...
2. **Memory Management**: The buffer size is limited to `MaxLogLines * 2` to avoid excessive memory usage
